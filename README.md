...
| No beacons on **Android 12+** | `getBluetoothState()` returns `unauthorized` — "Nearby devices" (`BLUETOOTH_SCAN`) was denied. **Granting location does NOT unlock BLE scan on 12+.** | Call `requestPermissions()` — it asks for `BLUETOOTH_SCAN` on Android — or, if the user already denied it permanently, send them to app settings. |
