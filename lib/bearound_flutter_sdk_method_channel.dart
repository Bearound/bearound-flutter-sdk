...
class MethodChannelBearoundFlutterSdk extends BearoundFlutterSdkPlatform {
  ...
  static const EventChannel _beaconsEventChannel = EventChannel(
    'bearound_flutter_sdk/beacons',
  );
  static const EventChannel _syncEventChannel = EventChannel(
    'bearound_flutter_sdk/sync',
  );
  static const EventChannel _regionEventChannel = EventChannel(
    'bearound_flutter_sdk/region',
  );
  ...
  Stream<BeaconsDetectedEvent> get beaconsStream {
    _beaconsStream ??= _beaconsEventChannel.receiveBroadcastStream().map((
      dynamic event,
    ) {
  ...
  Stream<BeaconEvent> get syncStream {
    _syncStream ??= _syncEventChannel.receiveBroadcastStream().map((
      dynamic event,
    ) {
      final map = Map<String, dynamic>.from(event as Map);
  ...
  Stream<BeaconEvent> get regionStream {
    _regionStream ??= _regionEventChannel.receiveBroadcastStream().map((
      dynamic event,
    ) {
      final map = Map<String, dynamic>.from(event as Map);
      return BeaconEvent.fromJson(map);
